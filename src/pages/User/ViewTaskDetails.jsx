...
const ViewTaskDetails = () => {
  ...
  const updateTodoChecklist = async (index) => {
    ...
    if (todoChecklist && todoChecklist[index]) {
      ...
      // Calculate new status based on updated todoChecklist
      const newStatus = getTaskStatus(todoChecklist);
      ...
      try {
        const response = await axiosInstance.put(
          ...
          { 
            todoChecklist,
            status: newStatus // Send the new status to backend
          }
        );
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 mt-4">
            <div className="form-card col-span-3">
              <div className="flex items-center justify-between">
                ...
                <div className={`text-[11px] md:text-[13px] font-medium ${getStatusTagColor(getTaskStatus(task?.todoChecklist))} px-4 py-0.5 rounded`}>
                  {getTaskStatus(task?.todoChecklist)}
                </div>
              </div>
  ...
  return (
    <div className="flex items-center gap-3 p-3">
      <input
        ...
        checked={isChecked}
        onChange={onChange}
        className="h-4 w-4 text-blue-500 bg-gray-100 border-gray-300 rounded-sm outline-none cursor-pointer"
      />
